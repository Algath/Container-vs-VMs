...
# the real demo code instead of a reimplementation of it.
#
...
echo "== Test 1: VM is defined and running, IP is reachable =="
vm_precheck
echo "OK (${VM_IP})"
...
echo "== Test 3: root SSH login is rejected (password auth disabled) =="
...
echo "== Test 4: vm_cold_start() runs the job and sets a timing =="
vm_cold_start
if [[ -z "${vm_start_ms:-}" ]]; then
    echo "FAIL: vm_start_ms was not set"
    exit 1
fi
echo "OK (${vm_start_ms} ms)"
